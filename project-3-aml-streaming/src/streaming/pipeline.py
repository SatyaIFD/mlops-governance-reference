...
def run_production_stream_pipeline():
    ...
    csv_path = next(
        (p for p in possible_csv_paths if p.exists()), possible_csv_paths[0]
    )
    ...
    for _ in range(50000):
        try:
            ...
            print("⚠️ Stream exhausted during Phase A warmup.")
            break
    ...
    print("[3/4] Phase B: Collecting high-fidelity mature events for training...")
    warmup_records = []
    for _ in range(50000):
        try:
            tx_raw = next(stream)
        except StopIteration:
            print("⚠️ Stream exhausted during Phase B collection.")
            break
    ...
    if not warmup_records:
        print(
            "🚨 FATAL: Insufficient stream data collected to train the model. Aborting."
        )
        return

    warmup_features_df = pd.DataFrame(warmup_records)
    warmup_features_df["Is_laundering"] = warmup_features_df["Is_laundering"].astype(
        int
    )

    laundering_in_train = int(warmup_features_df["Is_laundering"].sum())
    print(
        f" -> Laundering events present in high-fidelity training set: {laundering_in_train}"
    )
    ...
    for _ in range(records_to_score):
        ...
        if is_anomaly == 1:
            ...
            print(
                f"🚨 AML ALERT | Account: {tx['Sender_account']} ──> {tx['Receiver_account']} | Amt: ${float(tx['Amount']):.2f} "
    ...
    recall_pct = (
        (laundering_caught / total_laundering_in_stream) * 100
    ...
    print(f"Total Stream Transactions Processed: {actual_records_scored}")
