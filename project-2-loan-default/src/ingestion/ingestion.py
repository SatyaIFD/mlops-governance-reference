...
from src.ingestion.extract import LoanDataIngestionEngine  # noqa: E402
from src.ingestion.schemas import RAW_LOAN_DATA_SCHEMA  # noqa: E402
from src.ingestion.utils import initialize_directories, get_project_root  # noqa: E402
        ...
        if col in df.columns:
            if not pd.api.types.is_dtype_equal(df[col].dtype, expected_type):
                try:
                    ...
                    print(
                        f"❌ Type Cast Failure on column '{col}' to {expected_type}: {e}"
                    )
                    return False

    # 3. DEFENSIVE CI/CD FIX: Validate categorical boundaries to align with API expectations
    if "categorical_allowed_values" in RAW_LOAN_DATA_SCHEMA:
        for col, allowed_vals in RAW_LOAN_DATA_SCHEMA[
            "categorical_allowed_values"
        ].items():
            if col in df.columns:
                invalid_mask = ~df[col].isin(allowed_vals)
                if invalid_mask.any():
                    invalid_examples = df.loc[invalid_mask, col].unique()
                    print(
                        f"❌ Categorical Boundary Failure on column '{col}'. Found invalid values: {invalid_examples}. Expected only: {allowed_vals}"
                    )
                    return False

    print("✅ Raw dataset matches strict data contract rules.")
    return True
